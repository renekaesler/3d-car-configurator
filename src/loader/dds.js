...
export default class Dds extends Array {
  ...
  static parse(buffer) {
    ...
    const lod = headers.images.length / 6;
    const images = [];
    for(let i = 0; i < 6; ++i) {
      images[i] = [];
      for(let j = 0; j < lod; ++j) {
        const img   = headers.images[i * lod + j];
        const mipmap  = new Float32Array(buffer, img.offset, img.length / 4);
        mipmap.width = img.shape[0];
        mipmap.height = img.shape [1];

        images[i][j] = mipmap;
      }
    }

    return new Dds(images);
  }
...
function parseHeaders (arrayBuffer) {
  ...
  if (cubemap) {
    for (var f = 0; f < 6; f++) {
     ...
     // console.log(mipmapCount, requiredMipLevels)
